 ...
 * Сетка 6 колонок × 4 ряда: col1-3 = левая часть, col4-6 = правая часть.
 * Соответствует <c>EQUIP_INDEX_*</c> из <c>item.h</c>.
 *
 * ```
...
export interface MainSlotConfig {
  ...
  index: number
  row: number  // 1..4
  col: number  // 1..6 (col 1-3 левая часть, col 4-6 правая)
}
...
export const MAIN_LAYOUT: MainSlotConfig[] = [
  ...
  { index: 24, row: 1, col: 4 },
  { index: 1,  row: 1, col: 5 },
  { index: 19, row: 1, col: 6 },

  // Row 2
  { index: 2,  row: 2, col: 1 },
  { index: 3,  row: 2, col: 2 },
  { index: 18, row: 2, col: 3 },
  { index: 26, row: 2, col: 4 },
  { index: 4,  row: 2, col: 5 },
  { index: -1, row: 2, col: 6 },
  ...
  { index: 38, row: 4, col: 4 },
  { index: 7,  row: 4, col: 5 },
  { index: 11, row: 4, col: 6 },
]
